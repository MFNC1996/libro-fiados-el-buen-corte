...
v.e_monto.poner("15 mil")
v.anotar()
al_dia()
check("con fecha de ayer", v.datos.compras_de(cid)[-1]["fecha"] ==
      N.leer_fecha("ayer"))
check("15 mil = 15000", v.datos.deuda(cid) == 8990 + 15000)
v.e_monto.poner("abc")
v.e_detalle.poner("algo")
v.anotar()
check("un monto malo no se anota", v.datos.deuda(cid) == 8990 + 15000)
...
dlg.e_fecha.poner("ayer")
dlg._previa()
check("la previa dice cuanto queda debiendo",
      "Después del abono debe $13.990" in dlg.lbl_prev.cget("text"))
check("y que dia es", dlg.lbl_prev.cget("text").startswith("El "))
dlg.e_fecha.poner("martes 99")
dlg._previa()
check("una fecha mala se avisa", "No entiendo la fecha" in dlg.lbl_prev.cget("text"))
dlg.e_fecha.poner("ayer")
r = dlg.aceptar()
check("el abono queda con la fecha de ayer", r["fecha"] == N.leer_fecha("ayer"))
dlg.destroy()
v.recargar_todo()
al_dia()
